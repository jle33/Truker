...
namespace cecs521_website
{
    internal class db
    {
        ...
        public List<string>[] SelectDriversFromDist(string myquery)
        {
            ...
            if (this.OpenConnection() == true)
            {
                ...
                while (dataReader.Read())
                {
                    ...
                    list[5].Add(dataReader["latitude"] + "");
                    list[6].Add(dataReader["longitude"] + "");
                }
        ...
        public void xmlWriteTo(List<string>[] list)
        {
            ...
            using (XmlWriter writer = XmlWriter.Create(HttpContext.Current.Server.MapPath("locations.xml")))
            {
                ...
                for (int i = 0; i < size; i++)
                {
                    ...
                    writer.WriteAttributeString("name", list[0][i] + " " + list[1][i]);
                    //writer.WriteElementString("name", list[0][i] + " " + list[1][i]);
                    writer.WriteAttributeString("email", list[2][i]);
                    //writer.WriteElementString("email", list[2][i]);
                    writer.WriteAttributeString("phone", list[3][i]);
                    //writer.WriteElementString("phone", list[3][i]);
                    writer.WriteAttributeString("distance", list[4][i]);
                    //writer.WriteElementString("distance", list[4][i]);
                    writer.WriteAttributeString("lat", list[5][i]);
                    //writer.WriteElementString("lat", list[5][i]);
                    writer.WriteAttributeString("lng", list[6][i]);
                    //writer.WriteElementString("lng", list[6][i]);
